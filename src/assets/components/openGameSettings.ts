...
import formForReplaceWords from "./replace-words/formForReplaceWords";
...
function openGameSettings(e:Event) {
    ...
    if (target.classList.contains('ReplaceWords-button')){
        formForReplaceWords()
        openSection('ReplaceWords-form')
        window.history.pushState({ path }, path, path + '/ReplaceWords-form');
    }
